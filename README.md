...
- **とくせいと持ち物は、相性に効くものだけ扱う。**
  ...
  持ち物は143件を収録するが、相性に効くのは21件（半減きのみ18種＋ふうせん・くろいてっきゅう・ねらいのまと）だけ。
  残り122件はアイテム条項（同じ持ち物は1つまで）の検証用で、選んでもマトリクスは動かない。
  メガストーンは除外（メガを個体として持っているので二重管理になる）。プレート／メモリも除外（専用ポケモンがロースターにいない）。
  半減きのみは1回限りなので倍率には混ぜず、セルに `⇒` で併記するだけ（危険度判定にも含めない）。
- 持ち物は「第9世代に存在するもの」で絞っている。Champions 単位の持ち物データは PokéAPI に無いため。
- メガシンカ／ゲンシカイキは持ち物を持てない（メガストーン・オーブで枠が埋まるため）。
  アイテム条項の数え上げからも外れる。
- おすすめポケモンは「使用可の全員 × 相性に効くとくせい × 枠」を総当たりして順位付けする。
  同じ種で同じタイプ、および同じタイプ構成3件目以降は間引く（上位が同型で埋まらないように）。
