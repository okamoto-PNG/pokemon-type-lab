...
- **警戒すべき相手**は、使用可の全員を相手側に立たせて「自分の誰に刺さるか」を数えたもの。
  相手はタイプ一致技で殴ると仮定する。対策文はタイプ相性とすばやさから機械的に組み立てており、
  戦術を創作してはいない。「返せるが自分も抜群を受ける」場合は後出し不可として書き分ける。
- 種族値は収録済み（フォルム単位）。使うのは**すばやさ比較**と物理/特殊の偏り、同型の絞り込みまで。
- 技・持ち物・努力値・テラスタル・天候・ダメージ計算は扱わない。ダメージ計算機ではない。
